  ...
  class FlightList extends Component {
    ...
    componentWillReceiveProps(newprops) {
      ...
      if(this.props.searchString.searchId !== searchString.searchId) {
        ...
        this.props.actions.getLatestFlightResult(searchedQuery, allFlightList);
      }
    }
   ...
   render() {
     ...
     const { upFlightList, downFlightList, loadStatus, searchString } = this.props;
     ...
     return (
       <ListWrapper>
        ...
        { loadStatus === 1  && flightSearch && (<FlightResult>
            <div className="fullWidth">
              <h3 className="header">{searchString.originCity} > {searchString.destinationCity}</h3>
              { upFlightList.length !== 0 ? (<ul className="list-unstyle reduce-padding">
                { upFlightList.map((flight,index) => {
                    return <FlightDetails key={flight.id} flight={flight} passengers={searchString.passengerCount}  /> ;
                  })
                }
              </ul>) : <li className='emptyState'>
                <span>No Flight Found for Selected Date</span> <br/>
            ...
            { searchString.roundTrip &&
              <div className="fullWidth">
                <h3 className="header">{searchString.destinationCity} > {searchString.originCity}</h3>

                { downFlightList.length !== 0 ? (<ul className="list-unstyle reduce-padding">
                  { downFlightList.map((flight,index) => {
                      return <FlightDetails roundTrip key={flight.id} flight={flight} passengers={searchString.passengerCount}  />;
                    })
                  }
                </ul>) : <li className='emptyState'>
                  <span> No Flight Found for Selected Date </span> <br/>
