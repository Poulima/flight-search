...
import moment from 'moment';
  ...
  class SearchForm extends Component {
    ...
    searchOriginCity(searchTerm){
      ...
      const filterdList = originCityListCopy.filter( city => city.name.toLowerCase().includes(searchTerm.toLowerCase()) );
      this.setState({showOriginCityList: true,
                     showDestinationCityList: false,
    ...
    searchDestinationCity(searchTerm){
      ...
      this.setState({showOriginCityList: false,
                     originCityList:[],
    ...
    selectRoundTrip(e) {
      this.setState((prevState) => ({ roundTrip : true, returnDate: moment(prevState.departureDate).add(1, 'day')}));
    }
    ...
    changePassengerCount(event) {
      if(event.target.id === 'reduceButton') {
        this.setState((prevState) => ({ passengerCount: prevState.passengerCount - 1 }));
    ...
    handleSearch(event) {
      const { roundTrip, originCity, destinationCity, departureDate, returnDate, passengerCount, price } = this.state;
    ...
    render() {
      const { originCityList, destinationCityList, showOriginCityList, showDestinationCityList, roundTrip, originCity, destinationCity, departureDate, returnDate, passengerCount, price } = this.state;

      return (
        <FlightSearch>
          <form name="flightSearchForm" className="searchForm" onSubmit={this.handleSearch}>
            <SelectTrip
            ...
            <SelectCity
              ...
              showDestinationCityList={showDestinationCityList}
              selectedOriginCity={originCity}
              selectedDestinationCity={destinationCity}
              searchOriginCity={this.searchOriginCity}
              searchDestinationCity={this.searchDestinationCity}
              selectOriginCity={this.selectOriginCity}
              selectDestinationCity={this.selectDestinationCity}/>

            <SelectDate
              selectedDepartureDate={departureDate}
              changeDepartureDate={this.changeDepartureDate}
              selectedReturnDate={returnDate}
              changeReturnDate={this.changeReturnDate}
              roundTrip={roundTrip}
            />

            <PassengerCount maxPassenger={6} minPassenger={1}
            changePassengerCount={this.changePassengerCount} passengerCount={passengerCount}/>
            ...
            <div className='btn-wrapper'>
              <input type="submit" className='search-btn' value="Search" name="search_flight" />
            </div>
          </form>
        </FlightSearch>
      );
