 ...
 render() {
   ...
   return (
     <div className="container">
       <input type="text" name={name} className="cityInput" id={id} value={selectedCity} onChange={this.searchCityList} placeholder={placeholder} onBlur={this.clearValue} autoComplete="off" required />

       { showCityList && (
         <ul className='list-unstyle city-list'>
          { cityList.length !== 0 && cityList.map((city) => <EachCityItem key={city.id} city={city} selectCity={this.selectCity} />)}
          { cityList.length === 0 && <li className='emptyMsg'>No City Found</li>}
         </ul>
       )}
     </div>
   )}
